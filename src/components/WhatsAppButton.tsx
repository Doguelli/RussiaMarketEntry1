...
export default function WhatsAppButton() {
  ...
  // Determine language-specific prefilled message
  const getWhatsAppMessage = () => {
    if (i18n.language === "ru") {
      if (location.pathname.includes("kompaniya-v-turtsii")) {
        return t(
          "whatsapp.company_turkey_message",
          "Здравствуйте! Хочу проконсультироваться по поводу регистрации компании в Турции."
        );
      }
      return t(
        "whatsapp.default_message",
        "Здравствуйте! Хочу получить консультацию по выходу на рынок и услугам компании."
      );
    }
    ...
    if (i18n.language === "en") {
      return t(
        "whatsapp.default_message",
        "Hello! I would like to get information about Russia market entry and e-commerce consultancy."
      );
    }
    ...
    return t(
      "whatsapp.default_message",
      "Merhaba! Rusya pazarına giriş ve e-ticaret danışmanlığı hakkında bilgi almak istiyorum."
    );
  };

  const buttonText = t("whatsapp.button_text", i18n.language === "ru" ? "Написать в WhatsApp" : (i18n.language === "en" ? "Chat on WhatsApp" : "Hemen İletişime Geçin"));
  const ariaLabel = t("whatsapp.aria_label", i18n.language === "ru" ? "Написать в WhatsApp" : (i18n.language === "en" ? "WhatsApp Live Support" : "WhatsApp Canlı Destek"));
  const tooltip = t("whatsapp.tooltip", ariaLabel);
  const message = getWhatsAppMessage();
  const whatsappUrl = `https://wa.me/905327852420?text=${encodeURIComponent(message)}`;
